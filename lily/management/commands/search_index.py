...
class Command(BaseCommand):
    ...
    def handle_create(self, models, options, connection):
        """
        Create new indexes and point the aliases to them.
    ...
    def handle_delete(self, models, options, connection):
        """
        Delete the current indexes for the given models.
        ...
        if not options['force']:
            response = input(
                "Are you sure you want to delete the '{}' indexes? [n/Y]: ".format(", ".join(index_names))
            )
            if response.lower() != 'y':
    ...
    def handle_rebuild(self, models, options, connection):
        ...
        if not options['force']:
            answer = input("Are you sure you want to rebuild indexes '{}'? [n/Y]: ".format(", ".join(index_names)))
            if answer.lower() != 'y':
                self.stdout.write('Aborted')
                return False
