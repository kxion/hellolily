...
test:
	@echo "Make: docker-compose run --rm --service-ports web bash -c 'Dockers/wait-for db:5432 && python manage.py test'"
	@echo ""
	@docker-compose run --rm --service-ports web bash -c 'Dockers/wait-for db:5432 && python manage.py test'
	@echo ""
